...
public interface SysIndexExtendMapper {
    ...
    @Select("SELECT COUNT(1) as total, FROM_UNIXTIME(time/1000,'%m') t FROM sys_pro_detail where sys_pro_detail.is_active = 1 AND FROM_UNIXTIME(time/1000,'%Y年%m') LIKE CONCAT(#{year,jdbcType=VARCHAR},'%') GROUP BY t ORDER BY t")
    List<Map<String,String>> findYearPro(String year);

    @Select("SELECT COUNT(1) as total, FROM_UNIXTIME(time/1000,'%m') t FROM product_pool where product_pool.is_active = 1 AND FROM_UNIXTIME(time/1000,'%Y年%m') LIKE CONCAT(#{year,jdbcType=VARCHAR},'%') GROUP BY t ORDER BY t")
